          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {inspirationalDestinations.map((dest, index) => (
              <button
                ...
                className={`text-left p-4 rounded-lg border border-border/50 hover:border-border hover:bg-accent/50 transition-all duration-200 group playful-card animate-slide-in-up ${
                  selectedDestination === dest.name ? 'bg-blue-500/20 border-blue-400' : ''
                }`}
                style={{ animationDelay: `${0.1 + index * 0.05}s` }}
              >
                <div className="flex items-start space-x-3 relative">
                  <MapPin className={`w-5 h-5 text-blue-500 mt-0.5 group-hover:text-blue-400 transition-all duration-300 ${
                    hoveredDestination === dest.name ? 'animate-bounce-gentle' : ''
                  }`} />
                  <div className="flex-1">
                    ...
                    <span className={`inline-block mt-2 px-2 py-1 text-xs bg-blue-500/10 text-blue-600 dark:text-blue-400 rounded-full transition-all duration-300 ${
                      hoveredDestination === dest.name ? 'animate-glow-pulse' : ''
                    }`}>
                      {dest.category}
                    </span>
                  </div>
