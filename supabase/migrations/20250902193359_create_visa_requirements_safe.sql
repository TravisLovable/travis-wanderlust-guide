-- Safe creation of visa requirements table (only if it doesn't exist)
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_visa_requirements_origin_destination') THEN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_visa_requirements_destination_aliases') THEN
...
DO $$ BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'visa_requirements') THEN
...
DO $$ BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'visa_requirements') THEN
        IF NOT EXISTS (SELECT 1 FROM public.visa_requirements LIMIT 1) THEN
